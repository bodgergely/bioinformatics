...
vector<string> freqArrayTable(const string& text, int k) {
    ...
    for (int i = 0; i < text.size() - k + 1; i++) {
        ...
        // cout << loc << " ";
        array[loc]++;
        if (array[loc] > maxc) maxc = array[loc];
    }
...
vector<string> dummy(const string& text, int k) { return vector<string>(); }
...
vector<uint64_t> findOccurences(const char* pattern, int patternLen,
    ...
    for (uint64_t i = 0; i < textLen - k + 1; i++) {
        if (!memcmp(text + i, pattern, k)) occurences.push_back(i);
    }
    return occurences;
}
...
vector<uint64_t> approximatePatternMatch(const char* pattern, int patternLen,
                                         ...
                                         int mismatchLimit) {
    if (patternLen > textLen) return vector<uint64_t>();
...
vector<string> generateSimilar(const char* pattern, int patternLen,
    ...
    for (int i = 0; i < tableSize; i++) {
        if (table[i]) res.push_back(numToPattern(i, patternLen));
    }
    delete[] buffer;
    delete[] table;
 ...
 */
pair<vector<string>, int> findMostFreqKmersWithMismatches(
    const char* text, uint64_t size, int k, int d, bool reverseIncluded) {
    int freqArraySize = 1 << (2 * k);
    int* fa = freqArray(k);
...
public:
    ...
    vector<string> clumps(int k, int L, int t, uint64_t searchwindowSize = 0,
                          uint64_t startpos = 0) {
        if (searchwindowSize == 0) searchwindowSize = _genome.size();
        ...
        int backLoc = prevLoc;
        temp[prevLoc]++;
        if (temp[prevLoc] >= t) clump[prevLoc] = 1;

        for (int i = 1; i < L - k + 1; i++) {
            int loc = fastNextPos(prevLoc, k, _nucleoLookup[text[i + k - 1]]);
            temp[loc]++;
            if (temp[loc] >= t) clump[loc] = 1;
            prevLoc = loc;
        }
    ...
    size_t size() const { return _genome.size(); }
